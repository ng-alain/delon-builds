...
function fixHmr(sourceRoot, tree, context) {
    log_1.logStart(context, `Using built-in hmr instead of '@angularclass/hmr'`);
    // 检查是否存在 `hmr.ts` 文件作为是否已经安装 hmr 的前提条件
    const hmrTsPath = core_1.normalize(`${sourceRoot}/hmr.ts`);
    if (!tree.exists(hmrTsPath)) {
        log_1.logWarn(context, `Ingored the part migration when not found '${hmrTsPath}'`);
        return;
    }
    ...
    log_1.logInfo(context, `Remove '@angularclass/hmr'`);
    ...
    log_1.logInfo(context, `Removed 'environments.hmr.ts' file`);
    ...
    log_1.logInfo(context, `'-c=hmr' instead of '--hmr' in package.json`);
    // 4、修改 `main.ts` 并移除 `hmr.ts` 文件
    tree.delete(hmrTsPath);
    tree.overwrite(core_1.normalize(`${sourceRoot}/main.ts`), main_1.default);
    log_1.logInfo(context, `Modify 'main.ts' and remove 'hmr.ts' file`);
}
