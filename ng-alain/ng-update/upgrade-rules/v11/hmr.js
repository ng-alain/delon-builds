...
function removeHmrOfAngularJson(context) {
    return (0, workspace_1.updateWorkspace)((workspace) => __awaiter(this, void 0, void 0, function* () {
        ...
        (0, utils_1.logInfo)(context, `Remove '@angularclass/hmr'`);
    }));
}
function removeHmrInEnt(sourceRoot, name) {
    return (tree) => {
        const src = (0, core_1.normalize)(`${sourceRoot}/environments/${name}`);
        if (!tree.exists(src))
            return;
        const source = (0, utils_1.getSourceFile)(tree, src);
        const hmrNode = (0, ast_utils_1.findNode)(source, ts.SyntaxKind.Identifier, 'hmr');
        if (hmrNode == null || hmrNode.parent == null || hmrNode.parent.kind !== ts.SyntaxKind.PropertyAssignment) {
            return;
        }
        const content = (0, utils_1.readContent)(tree, src);
...
function removeHmrTs(sourceRoot) {
    return (tree, context) => {
        ...
        const environmentHmrTsPath = (0, core_1.normalize)(`${sourceRoot}/environments/environment.hmr.ts`);
        if (tree.exists(environmentHmrTsPath)) {
            tree.delete(environmentHmrTsPath);
        }
        (0, utils_1.logInfo)(context, `Removed 'environments.hmr.ts' file`);
    };
}
function fixPackage() {
    return (tree, context) => {
        ...
        (0, utils_1.logInfo)(context, `'-c=hmr' instead of '--hmr' in package.json`);
    };
}
function fixMainTs(hmrTsPath, sourceRoot) {
    return (tree, context) => {
        tree.delete(hmrTsPath);
        tree.overwrite((0, core_1.normalize)(`${sourceRoot}/main.ts`), main_1.default);
        (0, utils_1.logInfo)(context, `Modify 'main.ts' and remove 'hmr.ts' file`);
    };
}
function fixHmr(sourceRoot) {
    return (tree, context) => {
        (0, utils_1.logStart)(context, `Using built-in hmr instead of '@angularclass/hmr'`);
        // 检查是否存在 `hmr.ts` 文件作为是否已经安装 hmr 的前提条件
        const hmrTsPath = (0, core_1.normalize)(`${sourceRoot}/hmr.ts`);
        if (!tree.exists(hmrTsPath)) {
            (0, utils_1.logWarn)(context, `Ingored the part migration when not found '${hmrTsPath}'`);
            return;
        }
        return (0, schematics_1.chain)([
            ...
            (0, utils_1.removeAllowedCommonJsDependencies)('@angularclass/hmr'),
