import { CommonModule, DOCUMENT, isPlatformServer, registerLocaleData } from "@angular/common";
import * as i0 from "@angular/core";
import { DestroyRef, Injectable, InjectionToken, Injector, LOCALE_ID, NgModule, Optional, PLATFORM_ID, Pipe, SkipSelf, Version, importProvidersFrom, inject, makeEnvironmentProviders, provideEnvironmentInitializer, signal } from "@angular/core";
import { BehaviorSubject, Observable, Subject, catchError, delay, filter, finalize, isObservable, map, of, share, switchMap, take, tap, throwError } from "rxjs";
import { ACLService } from "@delon/acl";
import { ALAIN_CONFIG, AlainConfigService } from "@delon/util/config";
...
var AlainI18nBaseService = class AlainI18nBaseService {
	...
	constructor() {
		this.cog = this.cogSrv.merge("themeI18n", { interpolation: ["{{", "}}"] });
	}
	mergeData(data) {
		if (!data) return;
	...
	flatData(data, parentKey) {
		...
		for (const key of Object.keys(data)) {
			...
			} else res[(key ? parentKey.concat(key) : parentKey).join(".")] = `${value}`;
		}
		return res;
	}
	fanyi(path, params) {
...
var AlainI18NServiceFake = class AlainI18NServiceFake extends AlainI18nBaseService {
	use(lang, data) {
...
var MenuService = class MenuService {
	...
	getDefaultRedirect(opt = {}) {
		...
		this.visit(this.menus, (item) => {
			if (typeof item.link !== "string" || item.link.length <= 0 || !item._aclResult || item._hidden === true) return;
			if (ret == null || ret.length <= 0 || item.link == opt?.redirectUrl) ret = item.link;
		});
		return ret;
	}
	visit(data, callback) {
		const inFn = (list, parentMenu, depth) => {
			for (const item of list) {
				...
				if (item.children && item.children.length > 0) inFn(item.children, item, depth + 1);
				else item.children = [];
			}
		};
		inFn(data, null, 0);
	...
	fixItem(item) {
		...
		if (item.badge) {
			if (item.badgeDot !== true) item.badgeDot = false;
			if (!item.badgeStatus) item.badgeStatus = "error";
		}
		...
		if (typeof item.icon === "string") {
			...
			} else if (/^https?:\/\//.test(item.icon)) type = "img";
			item.icon = {
				type,
				value
			};
	...
	resume(callback) {
		...
		this.visit(this.data, (item, parent, depth) => {
			...
			item._depth = depth;
			this.fixItem(item);
			if (parent && item.shortcut === true && parent.shortcutRoot !== true) shortcuts.push(item);
	...
	find(options) {
		...
		if (opt.key != null) return this.getItem(opt.key);
		let url = opt.url;
		let item = null;
		while (!item && url) {
			this.visit(opt.data ?? this.data, (i) => {
				if (!opt.last && item != null) return;
				if (opt.ignoreHide && i.hide) return;
				if (opt.cb) {
					const res = opt.cb(i);
					if (typeof res === "boolean" && res) item = i;
				}
				if (i.link != null && i.link === url) item = i;
			});
			if (!opt.recursive) break;
			if (/[?;]/g.test(url)) url = url.split(/[?;]/g)[0];
	...
	getItem(key) {
		...
		this.visit(this.data, (item) => {
			if (res == null && item.key === key) res = item;
		});
		return res;
	}
	...
	toggleOpen(keyOrItem, options) {
		let item = typeof keyOrItem === "string" ? this.find({ key: keyOrItem }) : keyOrItem;
		if (item == null) this.visit(this.menus, (i) => {
			i._selected = false;
...
const REP_MAX = 6;
const SPAN_MAX = 24;
var ResponsiveService = class ResponsiveService {
	cogSrv = inject(AlainConfigService);
	cog;
	constructor() {
		this.cog = this.cogSrv.merge("themeResponsive", { rules: {
			1: { xs: 24 },
			2: {
				xs: 24,
				sm: 12
		...
		if (Object.keys(this.cog.rules).map((i) => +i).some((i) => i < 1 || i > 6)) throw new Error(`[theme] the responseive rule index value range must be 1-6`);
	}
	genCls(count, defaultCol = 1) {
		const rule = { ...this.cog.rules[count > 6 ? 6 : Math.max(count, 1)] };
		const antColClass = "ant-col";
		const itemMaxSpan = 24 / defaultCol;
...
const LTR = "ltr";
const RTL = "rtl";
var RTLService = class RTLService {
...
var TitleService = class TitleService {
		...
		return of("").pipe(delay(this.DELAY_TIME), map(() => {
			...
			if (el) {
				...
				el.childNodes.forEach((val) => {
					if (!text && val.nodeType === 3) text = val.textContent.trim();
				});
	...
	setTitle(title) {
		...
		this.tit$ = of(title).pipe(switchMap((tit) => tit ? of(tit) : this.getByRoute()), switchMap((tit) => tit ? of(tit) : this.getByMenu()), switchMap((tit) => tit ? of(tit) : this.getByElement()), map((tit) => tit || this.default), map((title) => !Array.isArray(title) ? [title] : title), takeUntilDestroyed(this.destroy$)).subscribe((titles) => {
			let newTitles = [];
			if (this._prefix) newTitles.push(this._prefix);
			newTitles.push(...titles.filter((title) => !!title));
			if (this._suffix) newTitles.push(this._suffix);
			if (this._reverse) newTitles = newTitles.reverse();
...
var AlainI18NGuardService = class AlainI18NGuardService {
	...
	process(route) {
		...
		if (lang != null) this.i18nSrv?.use(lang);
		return of(true);
	}
	static ɵfac = i0.ɵɵngDeclareFactory({
		minVersion: "12.0.0",
...
var ModalHelper = class ModalHelper {
	...
	buildDrag(options, wrapCls) {
		...
		if (handelEl) handelEl.classList.add(`${CLS_DRAG}-HANDLE`);
		return createDragRef(this.injector, handelEl ?? modalEl).withHandles([handelEl ?? modalEl]).withBoundaryElement(wrapEl).withRootElement(modalEl);
	}
	create(comp, params, options) {
		...
		return new Observable((observer) => {
			...
			let width = "";
			if (size) {
				if (typeof size === "number") width = `${size}px`;
				else if ([
					"sm",
					"md",
					"lg",
					"xl"
				].includes(size)) cls.push(`modal-${size}`);
				else width = size;
			}
			if (includeTabs) cls.push(`modal-include-tabs`);
			...
			if (modalRef.componentInstance != null && useNzData !== true && params != null) Object.entries(params).forEach(([key, value]) => {
				const t = modalRef.componentInstance;
				const s = t[key]?.[SIGNAL];
				if (s != null) s.value = value;
				else t[key] = value;
			});
			modalRef.afterOpen.pipe(take(1), delay(modalOptions?.nzNoAnimation ? 25 : 341), tap(() => {
				if (dragOptions != null) dragRef = this.buildDrag(dragOptions, `.${dragWrapCls}`);
			})).subscribe(() => {
				if (focus == null) return;
				const btns = modalRef.getElement().querySelector(".ant-modal-confirm-btns, .modal-footer")?.querySelectorAll(".ant-btn");
				const btnSize = btns?.length ?? 0;
				let el = null;
				if (btnSize === 1) el = btns[0];
				else if (btnSize > 1) el = btns[focus === "ok" ? 1 : 0];
				if (el != null) {
					el.focus();
...
var DrawerHelper = class DrawerHelper {
	srv = inject(NzDrawerService);
	parentDrawer = inject(DrawerHelper, {
		optional: true,
		skipSelf: true
	});
	...
	create(title, comp, params, options) {
		...
		return new Observable((observer) => {
			...
			if (typeof size === "number") defaultOptions[drawerOptions.nzPlacement === "top" || drawerOptions.nzPlacement === "bottom" ? "nzHeight" : "nzWidth"] = options.size;
			else if (!drawerOptions.nzWidth) {
				defaultOptions.nzWrapClassName = `${drawerOptions.nzWrapClassName} drawer-${options.size}`.trim();
				delete drawerOptions.nzWrapClassName;
			}
			if (footer) defaultOptions.nzBodyStyle = { "padding-bottom": `${footerHeight + 24}px` };
			const ref = this.srv.create({
				...defaultOptions,
				...drawerOptions
			});
	...
	closeAll() {
		...
		while (i--) this.openDrawers[i].close();
	}
...
var _HttpClient = class _HttpClient {
	...
	parseParams(params) {
		...
		Object.keys(params).forEach((key) => {
			...
			if (paramValue instanceof Date && (dateValueHandling === "timestamp" || dateValueHandling === "timestampSecond")) paramValue = dateValueHandling === "timestamp" ? paramValue.valueOf() : Math.trunc(paramValue.valueOf() / 1e3);
...
function setParam(target, key = paramKey) {
	...
	if (typeof params === "undefined") params = target[key] = {};
	return params;
}
...
function makeParam(paramName) {
	return function(key) {
		return function(target, propertyKey, index) {
			...
			if (typeof tParams === "undefined") tParams = params[paramName] = [];
			tParams.push({
				key,
				index
			});
...
function genBody(data, payload) {
	if (Array.isArray(data) || Array.isArray(payload)) return Object.assign([], data, payload);
	return {
		...data,
		...payload
	};
	...
	return function(url = "", options) {
		return (_target, targetKey, descriptor) => {
			descriptor.value = function(...args) {
				...
				if (http == null) throw new TypeError(`Not found '_HttpClient', You can import 'AlainThemeModule' && 'HttpClientModule' in your root module.`);
				const baseData = setParam(this);
				const data = setParam(baseData, targetKey);
				let requestUrl = url ?? "";
				requestUrl = [baseData.baseUrl ?? "", requestUrl.startsWith("/") ? requestUrl.substring(1) : requestUrl].join("/");
				if (requestUrl.length > 1 && requestUrl.endsWith("/")) requestUrl = requestUrl.substring(0, requestUrl.length - 1);
				if (options.acl) {
					const aclSrv = injector.get(ACLService, null);
					if (aclSrv && !aclSrv.can(options.acl)) return throwError(() => ({
						url: requestUrl,
						status: 401,
				...
				if (method === "FORM") headers["content-type"] = "application/x-www-form-urlencoded";
				...
				return http.request(method, requestUrl, {
					...
					params: !supportedBody ? {
						...params,
						...payload
					} : params,
					headers: {
						...baseData.baseHeaders,
						...headers
					},
					...options
				});
...
var DelonLocaleService = class DelonLocaleService {
	defLocale = inject(DELON_LOCALE, { optional: true });
	_locale = zh_CN_default;
	change$ = new BehaviorSubject(this._locale);
	constructor() {
		this.setLocale(this.defLocale ?? zh_CN_default);
	}
	get change() {
		return this.change$.asObservable();
	}
	setLocale(locale) {
		if (this._locale && this._locale.abbr === locale.abbr) return;
		this._locale = locale;
		this.change$.next(locale);
	}
	valueSignal(key) {
		return toSignal(this.change.pipe(map(() => this.getData(key))), { initialValue: this._locale[key] });
	}
	get locale() {
		return this._locale;
	}
...
var DelonLocaleModule = class DelonLocaleModule {
	static ɵfac = i0.ɵɵngDeclareFactory({
		minVersion: "12.0.0",
		version: "22.2.0",
		ngImport: i0,
		type: DelonLocaleModule,
		deps: [],
...
function yn(value, opt) {
	...
	switch (mode) {
		...
		case "text":
			...
			break;
		default: html = value ? `<i ${CLS_YES} title="${yes}">${ICON_YES}</i>` : `<i ${CLS_NO} title="${no}">${ICON_NO}</i>`;
	}
	return html;
}
var YNPipe = class YNPipe {
...
var AlainThemeModule = class AlainThemeModule {
	constructor(iconSrv) {
	...
	static ɵfac = i0.ɵɵngDeclareFactory({
		...
		version: "22.2.0",
		ngImport: i0,
		type: AlainThemeModule,
		deps: [{ token: i1.NzIconService }],
		target: i0.ɵɵFactoryTarget.NgModule
	});
	static ɵmod = i0.ɵɵngDeclareNgModule({
		minVersion: "14.0.0",
...
function provideAlain(options) {
	...
	const provides = [
		...
		{
			provide: DELON_LOCALE,
			useValue: lang?.delon ?? zh_CN_default
		},
		DELON_LOCALE_SERVICE_PROVIDER,
	...
	if (lang) {
		...
		provides.push({
			provide: LOCALE_ID,
			useValue: lang.abbr
		}, provideNzI18n(lang.zorro), provideNzDateFnsAdapter(lang.date));
	}
	const i18nCls = options?.i18nClass;
	if (i18nCls) provides.push({
