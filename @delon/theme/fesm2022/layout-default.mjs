...
import { ChangeDetectionStrategy, Component, Directive, Injectable, NgModule, Renderer2, ViewEncapsulation, afterNextRender, booleanAttribute, computed, contentChildren, effect, inject, input, numberAttribute, output, signal, viewChild } from "@angular/core";
import { takeUntilDestroyed } from "@angular/core/rxjs-interop";
import { NavigationCancel, NavigationEnd, NavigationError, RouteConfigLoadEnd, RouteConfigLoadStart, Router, RouterLink, RouterModule } from "@angular/router";
...
var LayoutDefaultHeaderItemComponent = class LayoutDefaultHeaderItemComponent {
	...
	static ɵfac = i0.ɵɵngDeclareFactory({
		...
		version: "22.2.0",
		ngImport: i0,
		type: LayoutDefaultHeaderItemComponent,
		deps: [],
...
var LayoutDefaultService = class LayoutDefaultService {
	...
	constructor() {
		...
		settings.notify.pipe(filter((w) => w.type === "layout"), map(() => {
			const collapsed = settings.layout.collapsed;
			return settings.layout.direction === "rtl" ? collapsed ? "fold" : "unfold" : collapsed ? "unfold" : "fold";
		}), takeUntilDestroyed()).subscribe((type) => this.collapsedIcon.set(`menu-${type}`));
	}
	checkMedia(value) {
		if (value) this.settings.setLayout("collapsed", true);
	}
	setOptions(options) {
		this.options.set({
			...DEFAULT,
...
var LayoutDefaultHeaderComponent = class LayoutDefaultHeaderComponent {
	...
	static ɵfac = i0.ɵɵngDeclareFactory({
		...
		version: "22.2.0",
		ngImport: i0,
		type: LayoutDefaultHeaderComponent,
		deps: [],
		target: i0.ɵɵFactoryTarget.Component
	});
	static ɵcmp = i0.ɵɵngDeclareComponent({
		...
		host: { classAttribute: "alain-default__header" },
		ngImport: i0,
		template: `
...
var LayoutDefaultNavComponent = class LayoutDefaultNavComponent {
	doc = inject(DOCUMENT);
	win = inject(WINDOW);
	...
	disabledAcl = input(false, {
		...ngDevMode ? { debugName: "disabledAcl" } : /* istanbul ignore next */ {},
		transform: booleanAttribute
	});
	autoCloseUnderPad = input(true, {
		...ngDevMode ? { debugName: "autoCloseUnderPad" } : /* istanbul ignore next */ {},
		transform: booleanAttribute
	});
	recursivePath = input(true, {
		...ngDevMode ? { debugName: "recursivePath" } : /* istanbul ignore next */ {},
		transform: booleanAttribute
	});
	hideEmptyChildren = input(true, {
		...ngDevMode ? { debugName: "hideEmptyChildren" } : /* istanbul ignore next */ {},
		transform: booleanAttribute
	});
	openStrictly = input(...ngDevMode ? [void 0, { debugName: "openStrictly" }] : /* istanbul ignore next */ []);
	maxLevelIcon = input(3, {
		...ngDevMode ? { debugName: "maxLevelIcon" } : /* istanbul ignore next */ {},
	...
	constructor() {
		effect(() => {
			...
			if (openStrictly != null) this.menuSrv.openStrictly = openStrictly;
		});
		this.menuSrv.change.pipe(takeUntilDestroyed()).subscribe((data) => {
			this.menuSrv.visit(data, (i, _p, depth) => {
				...
				if (!i._aclResult) {
					if (this.disabledAcl()) i.disabled = true;
					else i._hidden = true;
				}
				const icon = i.icon;
				if (icon && icon.type === "svg" && typeof icon.value === "string") icon.value = this.sanitizer.bypassSecurityTrustHtml(icon.value);
			});
		...
		this.settings.notify.pipe(takeUntilDestroyed(), filter((t) => t.type === "layout" && t.name === "collapsed")).subscribe(() => this.clearFloating());
		afterNextRender(() => {
			this.underPad();
	...
	genSubNode(linkNode, item) {
		...
		const node = (item.badge ? linkNode.nextElementSibling.nextElementSibling : linkNode.nextElementSibling).cloneNode(true);
		node.id = id;
	...
	hideAll() {
		this.floatingEl.querySelectorAll(`.${FLOATINGCLS}`).forEach((node) => node.classList.remove(SHOWCLS));
	}
	calPos(linkNode, node) {
		...
		const spacing = 5;
		let offsetHeight = -5;
		if (docHeight < rect.top + node.clientHeight) offsetHeight = rect.top + node.clientHeight - docHeight + spacing;
		node.style.top = `${rect.top + scrollTop - offsetHeight}px`;
		if (this.dir() === "rtl") node.style.right = `${rect.width + spacing}px`;
		else node.style.left = `${rect.right + spacing}px`;
	}
	showSubMenu(e, item) {
	...
	to(item) {
		...
		if (item.externalLink) {
			if (item.target === "_blank") this.win.open(item.externalLink);
			else this.win.location.href = item.externalLink;
			return;
		}
	...
	closeSubMenu() {
		if (this.collapsed) this.hideAll();
	}
	openByUrl(url) {
	...
	fixHide(ls) {
		const inFn = (list) => {
			for (const item of list) if (item.children && item.children.length > 0) {
				inFn(item.children);
				if (!item._hidden) item._hidden = item.children.every((v) => v._hidden);
			}
		};
		inFn(ls);
	}
	...
	underPad() {
		if (this.autoCloseUnderPad() && this.isPad && !this.collapsed) setTimeout(() => this.openAside(true));
	}
	openAside(status) {
	...
	static ɵfac = i0.ɵɵngDeclareFactory({
		...
		version: "22.2.0",
		ngImport: i0,
		type: LayoutDefaultNavComponent,
		deps: [],
		target: i0.ɵɵFactoryTarget.Component
	});
	static ɵcmp = i0.ɵɵngDeclareComponent({
		...
		inputs: {
			disabledAcl: {
				classPropertyName: "disabledAcl",
				publicName: "disabledAcl",
				isSignal: true,
				isRequired: false,
				transformFunction: null
			},
			autoCloseUnderPad: {
				classPropertyName: "autoCloseUnderPad",
				publicName: "autoCloseUnderPad",
				isSignal: true,
				isRequired: false,
				transformFunction: null
			},
			recursivePath: {
				classPropertyName: "recursivePath",
				publicName: "recursivePath",
				isSignal: true,
				isRequired: false,
				transformFunction: null
			},
			hideEmptyChildren: {
				classPropertyName: "hideEmptyChildren",
				publicName: "hideEmptyChildren",
				isSignal: true,
				isRequired: false,
				transformFunction: null
			},
			openStrictly: {
				classPropertyName: "openStrictly",
				publicName: "openStrictly",
				isSignal: true,
				isRequired: false,
				transformFunction: null
			},
			maxLevelIcon: {
				classPropertyName: "maxLevelIcon",
				publicName: "maxLevelIcon",
				isSignal: true,
				isRequired: false,
				transformFunction: null
			}
...
var LayoutDefaultComponent = class LayoutDefaultComponent {
	router = inject(Router);
	msgSrv = inject(NzMessageService);
	settings = inject(SettingsService);
	doc = inject(DOCUMENT);
	srv = inject(LayoutDefaultService);
	headerItems = contentChildren(LayoutDefaultHeaderItemComponent, {
		...ngDevMode ? { debugName: "headerItems" } : /* istanbul ignore next */ {},
		descendants: false
	});
	opt = this.srv.options;
	layout = this.settings.layoutSignal;
	options = input(...ngDevMode ? [void 0, { debugName: "options" }] : /* istanbul ignore next */ []);
	asideUser = input(...ngDevMode ? [void 0, { debugName: "asideUser" }] : /* istanbul ignore next */ []);
	asideBottom = input(...ngDevMode ? [void 0, { debugName: "asideBottom" }] : /* istanbul ignore next */ []);
	nav = input(...ngDevMode ? [void 0, { debugName: "nav" }] : /* istanbul ignore next */ []);
	content = input(...ngDevMode ? [void 0, { debugName: "content" }] : /* istanbul ignore next */ []);
	customError = input(...ngDevMode ? [void 0, { debugName: "customError" }] : /* istanbul ignore next */ []);
	fetchingStrictly = input(false, {
		...ngDevMode ? { debugName: "fetchingStrictly" } : /* istanbul ignore next */ {},
		transform: booleanAttribute
	});
	fetching = input(false, {
		...ngDevMode ? { debugName: "fetching" } : /* istanbul ignore next */ {},
		transform: booleanAttribute
	});
	...
	processEv(ev) {
		if (!this.isFetching() && ev instanceof RouteConfigLoadStart) this.isFetching.set(true);
		if (ev instanceof NavigationError || ev instanceof NavigationCancel) {
			...
			if (err && ev instanceof NavigationError) this.msgSrv.error(err, { nzDuration: 3e3 });
			return;
		}
		if (!(ev instanceof NavigationEnd || ev instanceof RouteConfigLoadEnd)) return;
		if (this.isFetching()) setTimeout(() => {
			this.isFetching.set(false);
		}, 100);
	}
	static ɵfac = i0.ɵɵngDeclareFactory({
	...
	static ɵcmp = i0.ɵɵngDeclareComponent({
		...
		inputs: {
			...
			asideUser: {
				classPropertyName: "asideUser",
				publicName: "asideUser",
				isSignal: true,
				isRequired: false,
				transformFunction: null
			},
			asideBottom: {
				classPropertyName: "asideBottom",
				publicName: "asideBottom",
				isSignal: true,
				isRequired: false,
				transformFunction: null
			},
			...
			customError: {
				classPropertyName: "customError",
				publicName: "customError",
				isSignal: true,
				isRequired: false,
				transformFunction: null
			},
			fetchingStrictly: {
				classPropertyName: "fetchingStrictly",
				publicName: "fetchingStrictly",
				isSignal: true,
				isRequired: false,
				transformFunction: null
			},
			fetching: {
				classPropertyName: "fetching",
				publicName: "fetching",
				isSignal: true,
				isRequired: false,
				transformFunction: null
		...
		host: {
			...
			classAttribute: "alain-default"
		},
		queries: [{
			propertyName: "headerItems",
			predicate: LayoutDefaultHeaderItemComponent,
			isSignal: true
		}],
  ...
  `,
	...
	propDecorators: {
		headerItems: [{
			...
			args: [i0.forwardRef(() => LayoutDefaultHeaderItemComponent), {
				descendants: false,
				isSignal: true
			}]
		}],
...
var LayoutDefaultHeaderItemTriggerDirective = class LayoutDefaultHeaderItemTriggerDirective {
	static ɵfac = i0.ɵɵngDeclareFactory({
		minVersion: "12.0.0",
		version: "22.2.0",
		ngImport: i0,
		type: LayoutDefaultHeaderItemTriggerDirective,
		deps: [],
	...
	static ɵdir = i0.ɵɵngDeclareDirective({
		...
		host: { classAttribute: "alain-default__nav-item" },
		ngImport: i0
	});
};
i0.ɵɵngDeclareClassMetadata({
...
var LayoutDefaultTopMenuItemComponent = class LayoutDefaultTopMenuItemComponent {
	...
	static ɵfac = i0.ɵɵngDeclareFactory({
		...
		version: "22.2.0",
		ngImport: i0,
		type: LayoutDefaultTopMenuItemComponent,
		deps: [],
		target: i0.ɵɵFactoryTarget.Component
	});
	static ɵcmp = i0.ɵɵngDeclareComponent({
...
var LayoutDefaultModule = class LayoutDefaultModule {
	static ɵfac = i0.ɵɵngDeclareFactory({
		minVersion: "12.0.0",
		version: "22.2.0",
		ngImport: i0,
		type: LayoutDefaultModule,
		deps: [],
