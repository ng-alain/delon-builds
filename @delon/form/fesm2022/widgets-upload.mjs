...
var UploadWidget = class UploadWidget extends ControlUIWidget {
	static KEY = "upload";
	i;
	fileList = signal([], ...ngDevMode ? [{ debugName: "fileList" }] : /* istanbul ignore next */ []);
	btnType = "";
	ngOnInit() {
		...
		if (res.listType === "picture-card") this.btnType = "plus";
		if (res.type === "drag") {
			res.listType = null;
			this.btnType = "drag";
	...
	pureValue(files) {
		files.filter((file) => !file.url).forEach((file) => {
  ...
  `,
		...
		dependencies: [
			...
			},
			{
				...
				outputs: ["nzChange", "nzFileListChange"],
				exportAs: ["nzUpload"]
			},
			{
				kind: "ngmodule",
...
var UploadWidgetModule = class UploadWidgetModule {
