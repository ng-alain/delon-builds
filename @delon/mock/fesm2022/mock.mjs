...
import { delay as delay$1, from, isObservable, map, of, switchMap, throwError } from "rxjs";
...
var MockService = class MockService {
	...
	realApplyMock(data) {
		...
		Object.keys(data).forEach((key) => {
			...
			Object.keys(rules).forEach((ruleKey) => {
				...
				if (!(typeof value === "function" || typeof value === "object" || typeof value === "string")) throw Error(`mock value of [${key}-${ruleKey}] should be function or object or string, but got ${typeof value}`);
				...
				if ([
					...
					"PATCH",
					"OPTIONS"
				].indexOf(rule.method) === -1) throw Error(`method of ${key}-${ruleKey} is not valid`);
				const item = this.cached.find((w) => w.url === rule.url && w.method === rule.method);
				if (item) item.callback = rule.callback;
				else this.cached.push(rule);
			});
		});
	...
	genRule(key, callback) {
		...
		} else if (/(\([^)]+\))/i.test(url)) martcher = new RegExp(url, "i");
		return {
			url,
			martcher,
			segments,
	...
	getRule(method, url) {
		...
		if (ret.martcher) ret.martcher.exec(url).slice(1).map((value, index) => {
...
const mockInterceptor = (req, next) => {
	...
	if (!rule && !config.force) return next(req);
	let res$;
	switch (typeof rule.callback) {
		case "function": {
			...
			if (urlParams.length > 1) urlParams[1].split("&").forEach((item) => {
				const itemArr = item.split("=");
				const key = itemArr[0];
				const value = itemArr[1];
				if (Object.keys(mockRequest.queryString).includes(key)) {
					if (!Array.isArray(mockRequest.queryString[key])) mockRequest.queryString[key] = [mockRequest.queryString[key]];
					mockRequest.queryString[key].push(value);
				} else mockRequest.queryString[key] = value;
			});
	...
	res$ = res$.pipe(map((res) => {
		...
		const anyRes = res;
		if (anyRes.body) anyRes.body = deepCopy(anyRes.body);
		if (config.log) {
