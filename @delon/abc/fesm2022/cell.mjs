...
import { ChangeDetectionStrategy, Component, DestroyRef, Directive, ElementRef, Injectable, NgModule, Renderer2, ViewContainerRef, ViewEncapsulation, booleanAttribute, computed, effect, inject, input, makeEnvironmentProviders, model, provideEnvironmentInitializer, signal } from "@angular/core";
...
var CellService = class CellService {
	...
	get(value, options) {
		...
		if (widget?.type === "fn") res.result = widget.ref(value, opt);
		return (typeof value === "function" ? value(value, opt) : of(res.result)).pipe(map((text) => {
			...
			switch (type) {
				...
				case "html":
					...
					break;
				case "string": if (isSafeHtml) res.safeHtml = "safeHtml";
			}
			if ((type === "badge" || type === "tag") && dictData?.tooltip != null) res.options.tooltip = dictData.tooltip;
			if (opt.mask != null) res.result.text = formatMask(res.result.text, opt.mask);
			return res;
		}));
	}
...
var CellHostDirective = class CellHostDirective {
	...
	constructor() {
		effect(() => {
			...
			if (componentType == null) {
				if (typeof ngDevMode === "undefined" || ngDevMode) warn(`cell: No widget for type "${widget.key}"`);
				return;
			}
			this.vcr.clear();
...
var CellComponent = class CellComponent {
	srv = inject(CellService);
	...
	_unit = signal(void 0, ...ngDevMode ? [{ debugName: "_unit" }] : /* istanbul ignore next */ []);
	_res = signal(void 0, ...ngDevMode ? [{ debugName: "_res" }] : /* istanbul ignore next */ []);
	...
	constructor() {
		...
		effect(() => {
			...
			sub?.unsubscribe();
			sub = this.srv.get(v, o).pipe(take(1), takeUntilDestroyed(this.d$)).subscribe((res) => {
				this._res.set(res);
	...
	_link(e) {
		...
		const url = link?.url;
		if (url == null) return;
		if (/https?:\/\//g.test(url)) this.win.open(url, link?.target);
	...
	_showImg(img) {
		...
		this.imgSrv.preview(list.map((p) => ({ src: p })), config.previewOptions).switchTo(idx);
	}
	static ɵfac = i0.ɵɵngDeclareFactory({
		minVersion: "12.0.0",
		version: "22.2.0",
...
var CellModule = class CellModule {
	static ɵfac = i0.ɵɵngDeclareFactory({
		minVersion: "12.0.0",
		version: "22.2.0",
		ngImport: i0,
