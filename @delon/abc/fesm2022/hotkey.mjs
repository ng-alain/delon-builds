import * as i0 from "@angular/core";
import { Directive, ElementRef, Input, NgModule, NgZone, inject } from "@angular/core";
import { Platform } from "@angular/cdk/platform";
import { install, uninstall } from "@github/hotkey";
var HotkeyDirective = class HotkeyDirective {
	el = inject(ElementRef).nativeElement;
	ngZone = inject(NgZone);
	platform = inject(Platform);
	set hotkey(key) {
		if (!this.platform.isBrowser) return;
		this.ngZone.runOutsideAngular(() => install(this.el, key));
	}
	ngOnDestroy() {
		if (!this.platform.isBrowser) return;
		this.ngZone.runOutsideAngular(() => uninstall(this.el));
	}
	static ɵfac = i0.ɵɵngDeclareFactory({
		minVersion: "12.0.0",
		version: "22.2.0",
		ngImport: i0,
		type: HotkeyDirective,
		deps: [],
		target: i0.ɵɵFactoryTarget.Directive
	});
	static ɵdir = i0.ɵɵngDeclareDirective({
		minVersion: "14.0.0",
		version: "22.2.0",
		type: HotkeyDirective,
		isStandalone: true,
		selector: "[hotkey]",
		inputs: { hotkey: "hotkey" },
		ngImport: i0
	});
};
i0.ɵɵngDeclareClassMetadata({
	minVersion: "12.0.0",
	version: "22.2.0",
	ngImport: i0,
	type: HotkeyDirective,
	decorators: [{
		type: Directive,
		args: [{ selector: "[hotkey]" }]
	}],
	propDecorators: { hotkey: [{
		type: Input,
		args: ["hotkey"]
	}] }
});
const DIRECTIVES = [HotkeyDirective];
var HotkeyModule = class HotkeyModule {
	static ɵfac = i0.ɵɵngDeclareFactory({
		minVersion: "12.0.0",
		version: "22.2.0",
		ngImport: i0,
		type: HotkeyModule,
		deps: [],
		target: i0.ɵɵFactoryTarget.NgModule
	});
	static ɵmod = i0.ɵɵngDeclareNgModule({
		minVersion: "14.0.0",
		version: "22.2.0",
		ngImport: i0,
		type: HotkeyModule,
		imports: [HotkeyDirective],
		exports: [HotkeyDirective]
	});
	static ɵinj = i0.ɵɵngDeclareInjector({
		minVersion: "12.0.0",
		version: "22.2.0",
		ngImport: i0,
		type: HotkeyModule
	});
};
i0.ɵɵngDeclareClassMetadata({
	minVersion: "12.0.0",
	version: "22.2.0",
	ngImport: i0,
	type: HotkeyModule,
	decorators: [{
		type: NgModule,
		args: [{
			imports: DIRECTIVES,
			exports: DIRECTIVES
		}]
	}]
});
export { HotkeyDirective, HotkeyModule };

//# sourceMappingURL=hotkey.mjs.map