...
import { Component, Directive, NgModule, ViewContainerRef, inject, input, model, viewChild } from "@angular/core";
...
var RangePickerShortcutTplComponent = class RangePickerShortcutTplComponent {
	tpl = viewChild.required("tpl", ...ngDevMode ? [{ debugName: "tpl" }] : /* istanbul ignore next */ []);
	list = [];
	click(_) {}
	static ɵfac = i0.ɵɵngDeclareFactory({
		...
		version: "22.2.0",
		ngImport: i0,
		type: RangePickerShortcutTplComponent,
		deps: [],
...
i0.ɵɵngDeclareClassMetadata({
  ...
  `
	...
	propDecorators: { tpl: [{
		type: i0.ViewChild,
		args: ["tpl", { isSignal: true }]
	}] }
});
var RangePickerDirective = class RangePickerDirective {
	dom = inject(DomSanitizer);
	vcr = inject(ViewContainerRef);
	nativeComp = inject(NzDatePickerComponent, {
		host: true,
		optional: true
	});
	...
	shortcut = input(null, {
		...
		transform: (v) => {
			...
			if (typeof v !== "object") cog.enabled = toBoolean(v) !== false;
			this._shortcut = cog;
	...
	constructor() {
		if (typeof ngDevMode === "undefined" || ngDevMode) assert(!!this.nativeComp, `It should be attached to nz-range-picker component, for example: '<nz-range-picker [(ngModel)]="i.start" extend [(ngModelEnd)]="i.end" shortcut></nz-range-picker>'`);
	...
	overrideNative() {
		...
		dp.onChangeFn = (list) => {
			let start = null;
			let end = null;
			if (list.length > 0 && list.filter((w) => w != null).length === 2) [start, end] = fixEndTimeOfRange([list[0], list[1]]);
			this.start = start;
			this.end = end;
			oldOnChangeFn(start);
	...
	refreshShortcut() {
		...
		let extraFooter;
		if (!this.nativeComp || !enabled) extraFooter = void 0;
		else {
			if (!this.shortcutFactory) this.shortcutFactory = this.vcr.createComponent(RangePickerShortcutTplComponent);
	...
	static ɵdir = i0.ɵɵngDeclareDirective({
		...
		inputs: {
			...
			ngModelEnd: {
				classPropertyName: "ngModelEnd",
				publicName: "ngModelEnd",
				isSignal: true,
				isRequired: true,
				transformFunction: null
			}
		},
		outputs: { ngModelEnd: "ngModelEndChange" },
...
var DatePickerModule = class DatePickerModule {
	...
	static ɵmod = i0.ɵɵngDeclareNgModule({
		...
		imports: [
			...
			RangePickerShortcutTplComponent
		],
		exports: [RangePickerDirective, RangePickerShortcutTplComponent]
	});
