...
var PdfComponent = class PdfComponent {
	...
	set pi(val) {
		...
		if (this.pageViewer) this.pageViewer.scrollPageIntoView({ pageNumber: this._pi });
	}
	set showAll(val) {
		this._showAll = val;
		this.resetDoc();
	...
	set renderText(val) {
		...
		if (this.pageViewer) this.resetDoc();
	}
	...
	load() {
		...
		loadingTask.promise.then((pdf) => {
			...
			this.emit("loaded");
			if (!this.pageViewer) this.setupPageViewer();
			this.resetDoc();
			this.render();
		}, (error) => this.emit("error", { error })).then(() => this.setLoading(false));
	}
	resetDoc() {
		const pdf = this._pdf;
		if (!pdf) return;
	...
	render() {
		...
		if (this._rotation !== 0 || currentViewer.pagesRotation !== this._rotation) this.timeExec(() => {
			currentViewer.pagesRotation = this._rotation;
		});
		if (this.stickToPage) this.timeExec(() => {
	...
	getScale(viewportWidth, viewportHeight) {
		const borderSize = this.showBorders ? 18 : 0;
		const el = this.el;
		const containerWidth = el.clientWidth - borderSize;
		const containerHeight = el.clientHeight - borderSize;
		if (containerHeight === 0 || viewportHeight === 0 || containerWidth === 0 || viewportWidth === 0) return 1;
		let ratio;
		switch (this.zoomScale) {
			...
			case "page-height":
				...
				break;
			default: ratio = containerWidth / viewportWidth;
		}
	...
	setupMultiPageViewer() {
		...
		const linkService = this.multiPageLinkService = new VIEWER.PDFLinkService({ eventBus });
	...
	setupSinglePageViewer() {
		...
		const linkService = this.singlePageLinkService = new VIEWER.PDFLinkService({ eventBus });
	...
	ngOnChanges(changes) {
		if (this.inited && !changes.src) this.render();
	}
	ngOnDestroy() {
		this.destroy();
	}
	...
	static ɵcmp = i0.ɵɵngDeclareComponent({
		...
		host: { properties: { "class.d-block": "true" } },
		exportAs: ["pdf"],
		usesOnChanges: true,
		ngImport: i0,
		template: `
