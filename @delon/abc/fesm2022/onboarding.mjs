...
import { ApplicationRef, ChangeDetectionStrategy, Component, ElementRef, EventEmitter, Injectable, InjectionToken, Injector, NgModule, ViewEncapsulation, afterNextRender, createComponent, effect, inject, runInInjectionContext, signal, viewChild } from "@angular/core";
import { Router } from "@angular/router";
import { debounceTime, delay, fromEvent, of, pipe, switchMap } from "rxjs";
...
var OnboardingComponent = class OnboardingComponent {
	...
	max = 0;
	op = new EventEmitter();
	running = signal(false, ...ngDevMode ? [{ debugName: "running" }] : /* istanbul ignore next */ []);
	dir = "ltr";
	popover = viewChild.required(NzPopoverDirective, ...ngDevMode ? [{ debugName: "popover" }] : /* istanbul ignore next */ []);
	get first() {
	...
	getLightData() {
		...
		const offsetWH = needPadding ? 16 : 0;
		return {
			top: top - offsetPos,
			left: left - offsetPos,
	...
	constructor() {
		...
		effect(() => {
			if (!this.running()) runInInjectionContext(this.injector, () => {
				afterNextRender(() => {
					this.updatePosition();
				});
			});
		});
		fromEvent(window, "resize").pipe(takeUntilDestroyed(), debounceTime(100)).subscribe(() => this.updatePosition());
	}
	scroll(pos) {
	...
	updatePosition() {
		...
		if (pos == null) {
			if (typeof ngDevMode === "undefined" || ngDevMode) console.warn(`Did not matches selectors [${this.item.selectors}]`);
			return;
		}
	...
	updatePrevElStatus(status) {
		if (this.prevSelectorEl) this.prevSelectorEl.classList[status ? "add" : "remove"]("onboarding__light-el");
	}
	to(type) {
		this.op.emit(type);
	}
	...
	static ɵcmp = i0.ɵɵngDeclareComponent({
		...
		host: {
			...
			classAttribute: "onboarding"
		},
		viewQueries: [{
			propertyName: "popover",
			first: true,
			predicate: NzPopoverDirective,
			descendants: true,
			isSignal: true
		}],
		ngImport: i0,
		...
		dependencies: [
			{
				...
				outputs: ["nzPopoverVisibleChange"],
				exportAs: ["nzPopover"]
			},
			{
				kind: "directive",
...
i0.ɵɵngDeclareClassMetadata({
  ...
  `,
	...
	propDecorators: { popover: [{
		type: i0.ViewChild,
		args: [i0.forwardRef(() => NzPopoverDirective), { isSignal: true }]
	}] }
});
...
var OnboardingService = class OnboardingService {
	...
	attach() {
		const compRef = createComponent(OnboardingComponent, { environmentInjector: this.appRef.injector });
		this.compRef = compRef;
		this.appRef.attachView(compRef.hostView);
		const compNode = compRef.hostView.rootNodes[0];
		const doc = this._getDoc();
		const cdk = doc.querySelector(".cdk-overlay-container");
		if (cdk) doc.body.insertBefore(compNode, cdk);
		else doc.body.appendChild(compNode);
	...
	destroy() {
		...
		if (storeKey != null) this.keyStoreSrv.set(storeKey, this.config?.keyVersion);
		this.cancelRunning();
		if (this.compRef) {
	...
	start(config) {
		...
		if (storeKey != null && this.keyStoreSrv.get(storeKey) === cog.keyVersion) return;
	...
	prev() {
		if (this._running || this.active - 1 < 0) return;
		this.type = "prev";
		--this.active;
		this.showItem();
	}
...
var OnboardingModule = class OnboardingModule {
	static ɵfac = i0.ɵɵngDeclareFactory({
		minVersion: "12.0.0",
		version: "22.2.0",
		ngImport: i0,
