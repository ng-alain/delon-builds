...
import { ChangeDetectionStrategy, Component, ElementRef, Injector, NgModule, ViewEncapsulation, afterNextRender, booleanAttribute, effect, inject, input, numberAttribute, runInInjectionContext, signal, viewChild } from "@angular/core";
import { DomSanitizer } from "@angular/platform-browser";
import { NzTooltipDirective, NzTooltipModule } from "ng-zorro-antd/tooltip";
var EllipsisComponent = class EllipsisComponent {
	el = inject(ElementRef).nativeElement;
	injector = inject(Injector);
	dom = inject(DomSanitizer);
	doc = inject(DOCUMENT);
	isSupportLineClamp = this.doc.body.style["webkitLineClamp"] !== void 0;
	orgEl = viewChild.required("orgEl", ...ngDevMode ? [{ debugName: "orgEl" }] : /* istanbul ignore next */ []);
	...
	fullWidthRecognition = input(false, {
		...ngDevMode ? { debugName: "fullWidthRecognition" } : /* istanbul ignore next */ {},
		transform: booleanAttribute
	});
	...
	cutStrByFullLength(str, maxLength) {
		...
		return str.split("").reduce((pre, cur) => {
			const charCode = cur.charCodeAt(0);
			if (charCode >= 0 && charCode <= 128) showLength += 1;
			else showLength += 2;
	...
	bisection(targetHeight, mid, begin, end, text, node) {
		...
		if (sh <= targetHeight) {
			...
			if (sh > targetHeight || mid === begin) return mid;
			begin = mid;
		...
		node.innerHTML = text.substring(0, mid - 1) + suffix;
		sh = node.offsetHeight;
		if (sh <= targetHeight) return mid - 1;
		end = mid;
	...
	genType() {
		...
		if (!lines && !length) this.type.set("default");
		else if (!lines) this.type.set("length");
		else if (isSupportLineClamp) this.type.set("line-clamp");
		else this.type.set("line");
	}
	gen() {
		const lines = this.lines();
		const length = this.length();
		...
		if (type === "length") {
			...
			if (length == null || textLength <= length || length < 0) this.text.set(lengthText);
			else {
				let displayText;
				if (length - tail().length <= 0) displayText = "";
			...
			const targetHeight = lines * parseInt(this.win.getComputedStyle(this.getEl(".ellipsis")).lineHeight, 10);
	...
	static ɵcmp = i0.ɵɵngDeclareComponent({
		...
		inputs: {
			...
			fullWidthRecognition: {
				classPropertyName: "fullWidthRecognition",
				publicName: "fullWidthRecognition",
				isSignal: true,
				isRequired: false,
				transformFunction: null
			},
			tail: {
				classPropertyName: "tail",
				publicName: "tail",
				isSignal: true,
				isRequired: false,
				transformFunction: null
		...
		viewQueries: [
			...
			{
				propertyName: "shadowOrgEl",
				first: true,
				predicate: ["shadowOrgEl"],
				descendants: true,
				isSignal: true
			},
			{
				propertyName: "shadowTextEl",
				first: true,
				predicate: ["shadowTextEl"],
				descendants: true,
				isSignal: true
			}
		],
...
i0.ɵɵngDeclareClassMetadata({
  ...
  `,
	...
	propDecorators: {
		orgEl: [{
			type: i0.ViewChild,
			args: ["orgEl", { isSignal: true }]
		}],
		shadowOrgEl: [{
			type: i0.ViewChild,
			args: ["shadowOrgEl", { isSignal: true }]
		}],
		shadowTextEl: [{
			type: i0.ViewChild,
			args: ["shadowTextEl", { isSignal: true }]
		}],
		tooltip: [{
			type: i0.Input,
			args: [{
...
var EllipsisModule = class EllipsisModule {
	static ɵfac = i0.ɵɵngDeclareFactory({
		minVersion: "12.0.0",
		version: "22.2.0",
		ngImport: i0,
